...
const nextConfig: NextConfig = {
  ...
  experimental: {
    ...
    // ── זיכרון הניווט בצד הלקוח ──
    // ברירת המחדל של Next ל-dynamic היא אפס שניות: כל מסך במערכת הזו מסומן
    // force-dynamic, ולכן *כל* חזרה למסך שכבר ראית — כולל כפתור "אחורה" —
    // נשלפה מחדש מהשרת במלואה. מעבר בין שתי לשוניות הלוך ושוב שילם על אותן
    // שאילתות פעמיים.
    //
    // שלושים שניות: מספיק כדי שמעבר בין מסכים סמוכים יהיה מיידי, וקצר מכדי
    // שמישהו יעבוד על נתון ישן. פעולה שמשנה משהו קוראת ל-revalidatePath
    // וזו מנקה את הזיכרון הזה מיד, ולכן אחרי שמירה תמיד רואים את החדש.
