 ...
 *
 * זו בלימה של ה*קצב*, כדי ששליחה גדולה תתפרס על כמה ריצות במקום לפרוץ החוצה
 * בבת אחת. היא אינה המכסה של ספק הדואר, שהיא יומית.
 *
 * ⚠ **התקרה הזו כמעט אף פעם אינה הגורם המגביל — תקציב הזמן הוא.** הלולאה
 * למטה עוצרת כשנותרו פחות מ-SEND_ALLOWANCE_MS עד ה-deadline, ולכן מה שקובע
 * בפועל הוא כמה שניות המנוע קיבל מהקרון. כל חישוב של "כמה ייצאו בשעה" חייב
 * להתחיל משם ולא מכאן: המספר שכתוב פה הוא רק החסם העליון.
 *
 * (עד לתיקון של 5.9.2026 הניוזלטר קיבל חמישית מהחלון — תשע שניות — כך
 * שהתקרה הזו לא הייתה מושגת לעולם, ורשימה של 600 איש נמרחה על יום שלם. ראו
 * את חלוקת הזמן ב-app/api/cron/check-rules/route.ts.)
 */
