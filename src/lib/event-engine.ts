 ...
 * האירוע כמעט כל הנרשמים כבר מחוצה לו. הגרסה הראשונה של הקובץ הזה שלחה
...
export async function runEventReminders(
  ...
  for (const reminder of reminders as EventReminder[]) {
    ...
    // רק מי ששילם. מתעניין שלא סגר לא אמור לקבל "נתראה מחר", ובבסיס
    // purchase ממילא אין לו paid_at לספור ממנו.
    ...
    for (const row of paid ?? []) {
      ...
      const result = await sendMessageToContact({
        ...
        // ביומן, כדי שמי שקורא אותו יראה מה הלקוח קיבל בפועל.
...
/**
 * מתי התזכורת הזו אמורה לצאת לנרשם הזה.
 *
 * בבסיס event זה אותו רגע לכולם; בבסיס purchase לכל אחד שעון משלו, שמתחיל
 * ברגע התשלום. רשומה שסומנה כשילם ידנית לפני שהעמודה מולאה מחזירה null —
