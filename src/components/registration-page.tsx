 ...
 * מה שהלקוח רואה בפועל, וזה בדיוק סוג הפער שמתגלה רק אחרי שהקהל כבר קיבל
 * את הקישור.
 *
...
function FormCard({
  ...
  return (
    <>
      ...
      {/* לפני הכפתור ולא אחרי הטופס: מי שמגלה שהוא צריך לשלם רק אחרי
          שמילא שדות מרגיש שמשכו אותו. במצב "מלא" אין תשלום — יש המתנה. */}
...
function useReportHeight(embedId: string) {
  ...
  useEffect(() => {
    ...
    // שקט כאן פירושו כפתור שליחה חתוך אצל לקוח אמיתי. לכן הוא ראשון —
    // ולא יחיד.
...
export function RegistrationEmbed({
  ...
  const handleResult = (state: RegisterState) => {
    ...
    try {
      ...
      // פעולה של המשתמש — וזו בדיוק לחיצה על כפתור השליחה.
 ...
 * ולכן אין לו גישה ל-session, לפרמטרים מהטופס או לזהות הנרשם. כל מה שהוא
