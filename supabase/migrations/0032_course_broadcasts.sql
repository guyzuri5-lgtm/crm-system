-- ── שליחה חד-פעמית לנרשמי קורס ────────────────────────────────────────────
--
-- מה שלא היה קיים עד כאן: דרך *ידנית* לפנות למי שנרשם לקורס. שלושת המנועים
-- הקיימים כולם אוטומטיים ואף אחד מהם לא מכסה את זה — מסע נכנסים אליו רק
-- "מתעניינים", כלל אוטומציה מגיב לשינוי סטטוס של איש קשר (וההרשמה לקורס
...
create table course_broadcasts (
  ...
  -- הנמענים עצמם כבר קפואים ב-course_broadcast_recipients, ומי שיעבור שלב
  -- מחר לא אמור לשנות למפרע את מה שכתוב שנשלח אליו.
  ...
  -- היה כותב בו, והלקוח היה מקבל משהו אחר. במייל אין מגבלה כזו, ולכן שם
...
-- שיירשם בזמן שהשליחה באוויר לא ייכנס אליה באמצע — וזה הנכון: "שלחתי ל-47"
...
comment on table course_broadcasts is
  'שליחה חד-פעמית וידנית לנרשמי קורס. מייל = תוכן חופשי, וואטסאפ = תבנית מאושרת.';
comment on table course_broadcast_recipients is
  'תמונת מצב של הנמענים ברגע הלחיצה. המפתח המורכב מונע שליחה כפולה.';
