...
-- ── מי נרשם ────────────────────────────────────────────────────────────────
--
-- שלושה שלבים ולא דגל "שילם/לא שילם": מי שהשאיר פרטים ולא הגיע לתשלום הוא
-- לא כישלון אלא ליד — הוא מי שהמסע למתעניינים מדבר אליו. השלב עולה בדרגה
-- בלבד (interested → registered → paid), כדי שרישום חוזר לא יוריד מישהו
-- ששילם בחזרה למתעניין.
...
-- שעה, וחלון "בעוד 50–70 דקות" נפתח לכמה ריצות — בלי הרשומה הזו אותו נרשם
-- היה מקבל את אותה תזכורת שלוש פעמים. המפתח הראשי הוא המונע, לא הבדיקה
