...
- סגנון: **שפת "פטינה"**, שהוחלפה לתוך המערכת בשלבים 0–5 (ראה `תכנית עבודה עיצוב.md` ו-`המשך עבודה עיצוב.md`). השתמש במחלקות שכבר קיימות ב-`globals.css` — `card`, `card-h/b/f`, `btn-*`, `input`, `field-label`, `table-wrap`, `th`, `td`, `tr-hover`, `cbx`, `glyph`, `pill`, `av`, `tag`, `slug`, `h-page`, `page-title`, `tab` ועוד — ובאסימוני ה-CSS. **אל תמציא מערכת עיצוב חדשה.**
- **אפס צבעים קשיחים.** לא `bg-white`, ולא מחלקות Tailwind כמו `bg-emerald-50` או `text-gray-500`: המצב הכהה דלוק, והן אינן מגיבות לו. הכול דרך האסימונים (`--surface`, `--muted`, `--ok`, `--warn`, `--danger`, `--nav-*`).
...
| 2. דף בית | ✅ בפרודקשן | — | — |
| 3. ניוזלטר | ✅ בפרודקשן, **נבדק מקצה לקצה** (5.9.2026) | — | 0022, 0023 |
...
**החסם בשלב 3 הוסר (5.9.2026).** חשבון Postmark אושר ופעיל (`DeliveryType:
Live`), וניוזלטר אמיתי יצא בפרודקשן בזרם broadcast עם מסירה מאושרת. ~~ברגע
שהחשבון ישוחרר, השליחה תעבוד~~
בלי שינוי קוד.
...
- `/events/[id]`: מסך הניהול — שלושה מוני מדד למעלה, טבלת אנשים עם תגי שלב צבעוניים (שילם=ירוק soft, מתעניין=amber soft, לא שילם=pink soft) ועמודת מקור, כפתורים: "העתק לינק הרשמה" (ללוח), "עיצוב הדף" (לעורך), "סמן כשילם" ידני לכל שורה (fallback לגרואו), ו"מסע למתעניינים" (ראה 4ה).
...
קרא איך מוגדרים טריגרי הכניסה של מסעות (`src/app/(dashboard)/journeys` + `src/lib/journey-engine.ts`) והוסף סוג טריגר כניסה חדש: "נרשם כמתעניין לאירוע" עם בחירת אירוע — באותו דפוס בדיוק כמו הטריגרים הקיימים, כולל בורר בממשק המסעות. כשנוצרת `event_registrations` עם stage=interested, צרף את איש הקשר למסעות שמאזינים לאירוע הזה (באותו מקום בקוד שבו טריגרים קיימים מצרפים). הכפתור "מסע למתעניינים" במסך האירוע מוביל ל-`/journeys` עם פרמטר שפותח יצירת מסע עם הטריגר הזה מסומן מראש.
...
- `/courses`, `/courses/new`, `/courses/[id]`, `/courses/[id]/edit` — שיקוף מדויק של מסכי האירועים, כולל עורך העיצוב עם התצוגה החיה (בלי תזכורות, קיבולת, מתגי תאריך/מקומות וכפתור היומן). מוני המדד: "לקוחות בקורס" (paid), "מתעניינים", "התחילו ולא שילמו".
- טריגר כניסה למסע: "נרשם כמתעניין לקורס" — הרחב את מה שנבנה ב-4ה באותו אופן.
- דאשבורד: בשורת הסטטוס התחתונה החלף את הכרטיס השלישי ב"מתעניינים בקורסים" (סה"כ interested פתוחים + כמה חדשים השבוע, כחול). הוסף ל"דורש טיפול": מתעניינים (אירוע או קורס) שלא מצורפים לאף מסע.
...
שלח POST-ים מדומים לשני ה-endpoints עם curl וודא: שמירה ב-inbox, יצירת מתעניין ממטא עם mapping, סימון paid מגרואו, ודחיית קריאות בלי טוקן/סוד.
...
- ~~**אישור חשבון Postmark**~~ — ✅ אושר 5.9.2026, `DeliveryType: Live`.
- ~~**שליחת ניוזלטר ראשון אמיתי**~~ — ✅ נשלח 5.9.2026, מסירה אושרה.
